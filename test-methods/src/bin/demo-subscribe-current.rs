...
async fn test_subscribe_current(vehicle: &mut KuksaClient, signal: &str) {
...
async fn main() {
    ...
    test_subscribe_current(&mut vehicle, WRONG_SIGNAL).await;
    
    // status: PASSED
    // expected: error [signal not found] / [not a leaf entry]
    // result: error [signal not found]
    test_subscribe_current(&mut vehicle, BRANCH_SYSTEM_SIGNAL).await;

    // status: PASSED
    // expected: passed
    // result: passed (uint)
    test_subscribe_current(&mut vehicle, ACT_POSITION_SIGNAL).await;
}
